...
class Binance:
	...
	def GetTradingSymbols(self, quoteAssets:list=None, baseAssets:list=None):
		...
		for pair in data['symbols']:
			if pair['status'] == 'TRADING':
				...
				if quoteAssets != None and baseAssets == None:
					if pair['quoteAsset'] in quoteAssets:
						symbols_list.append((pair['symbol'], pair['baseAsset'], pair['quoteAsset']))

				if quoteAssets == None and baseAssets != None:
					if pair['baseAsset'] in baseAssets:
						symbols_list.append((pair['symbol'], pair['baseAsset'], pair['quoteAsset']))

				if quoteAssets != None and baseAssets != None:
					if pair['baseAsset'] in baseAssets and pair['quoteAsset'] in quoteAssets:
						symbols_list.append((pair['symbol'], pair['baseAsset'], pair['quoteAsset']))
	...
	def GetSymbolKlines(self, symbol:str, interval:str, limit:int=1000, end_time=False):
		...
		df = df.drop([6,8,9,10,11], axis=1)

		# rename columns
		col_names = ['time', 'open', 'high', 'low', 'close', 'volume','quoteAssetVolume']
	...
	def RoundToValidPrice(cls, symbol_data, desired_price, round_up:bool=False) -> Decimal:
		...
		#	if exit order, symbol_data is actually just a str of the symbol itself... need a way to round correctly when exiting... Tick size cant be found
		pr_filter = {}
