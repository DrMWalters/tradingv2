...
class BotRunner:
	...
	def EntryOrder(self, bot_params, strategy_function, pairs, symbol_data):
		...
		if buy is not False:
			...
			if self.ask_permission:
				...
				winsound.PlaySound('other//arrow_x.wav', winsound.SND_FILENAME)
				permission = input()
				sp.start()
				if permission != 'y':
					return
	...
	def ExitOrder(self, bot_params, pairs, order:dict):
		...
		pair = pairs[symbol]
		print(self.all_symbol_datas[symbol])
	...
	def OrderResultToDatabase(self, order_result, symbol_data, bot_params, is_entry_order=False, is_closed=False, closing_order_id=False):
		...
		order = dict()
		if symbol_data == None or symbol_data == []:
	...
	def StartExecution(self, bots):
		...
		if len(bots) == 0 or len(bots) == None:
		...
		if i == 'e':
			...
			if i == 'y':
				...
				for symbol in symbols:
					# sp.text = "Checking "+symbol[0]+"("+currentInterval+")..."
